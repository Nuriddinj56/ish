...
$coinbase = $pdo->query("SELECT * FROM exchanges WHERE what = 'Магазины'");
...
$time = time();
$method = "GET";
$res_data = $pdo->query("SELECT * FROM currency_api WHERE id != 4");
			while ($row = $res_data->fetch()) {
				...
				$headers = array(
					...
					$pdo->prepare("UPDATE currency_api SET kurs=? WHERE nominal = '".$currency."'")->execute(array($arr['data']['rates']['RUB']));
					$pdo->prepare("UPDATE currency_api SET kurs_usd=? WHERE nominal = '".$currency."'")->execute(array($arr['data']['rates']['USD']));
