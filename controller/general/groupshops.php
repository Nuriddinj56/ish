...
<h6 class="">Категории магазинов<span class="" style="font-size:12px;color:#888ea8;"><br>Добавление, редактирование, настройки.</span></h6>
</div></div>
...
<div class="widget-heading">
<h6 class="">Добавить категорию<span class="" style="font-size:12px;color:#888ea8;"><br>Быстрое добавление категории.</span></h6>
</div>
<hr style="margin-top:-25px;">
<div class="widget-content">
<form class="select mt-4" action="/inc/group/save.php" id="form" action="javascript:void(0);">
<div class="form-group">
										<input type="text" id="name" name="name" class="form-control form-control-sm" placeholder="Название категории" required />
										<small id="emailHelp" class="form-text text-muted">Название категории, обязательное поле.</small>
									  </div>
									  <div class="form-group">
									<div class="custom-progress progress-down" style="width: 100%">
									<small id="emailHelp" class="form-text text-muted">Укажите процент, который будет распространятся на всю группу магазинов в категории.</small>
...
<button class="btn btn-block btn-primary mt-2" id="btnSubmit" type="submit">Добавить категорию</button>
</form>
</div>
</div>
</div>
...
<div class="modal-header">
	<div class="widget-heading">
<h6 class="">Редактирование группы<span class="" style="font-size:12px;color:#888ea8;"><br>Внесите изменения.</span></h6>
</div>
	<button type="button" class="close" data-dismiss="modal">&times;</button>
</div>
<div class="modal-body" style="margin-top:-50px;">
		<form class="select mt-4" action="/inc/group/update.php" id="editform" action="javascript:void(0);">
...
var action = "activeRecord";
	$.ajax({
		...
		success:function(data) {					
			...
			Snackbar.show({
			text: 'Категория успешно включена',
			showAction: false,
			actionTextColor: '#fff',
...
var action = "inactiveRecord";
	$.ajax({
		...
		success:function(data) {					
			dataRecords.ajax.reload();
			Snackbar.show({
			text: 'Категория успешно выключена',
			showAction: false,
			actionTextColor: '#fff',
...
$("#recordListing").on('click', '.zamena', function(){
var id = $(this).attr("id");		
var action = "zamenaRecord";
	$.ajax({
		url:"/ajaxgroupshops/",
		method:"POST",
		data:{id:id, action:action},
		success:function(data) {					
			dataRecords.ajax.reload();
			Snackbar.show({
			text: 'Включен режим замены карты',
			showAction: false,
			actionTextColor: '#fff',
			backgroundColor: '#e7515a'
			});
...
$("#recordListing").on('click', '.zamenaoff', function(){
var id = $(this).attr("id");		
var action = "zamenaOffRecord";
	$.ajax({
		url:"/ajaxgroupshops/",
		method:"POST",
		data:{id:id, action:action},
		success:function(data) {					
			dataRecords.ajax.reload();
			Snackbar.show({
			text: 'Режим замены карты отключен',
			showAction: false,
			actionTextColor: '#fff',
			backgroundColor: '#1abc9c'
			});
...
url: '/inc/group/all.php', // get the route value
...
url: '/inc/group/get.php', // get the route value
...
$.ajax({
	type: "GET", //we are using GET method to get data from server side
	url: '/inc/group/delete.php', // get the route value
	data: {employee_id:employeeId}, //set data
	beforeSend: function () {//We add this before send to disable the button once we submit it so that we prevent the multiple click
	},
	success: function (response) {//once the request successfully process to the server side it will return result here
