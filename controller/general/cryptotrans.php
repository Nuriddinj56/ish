...
<h6 class="">Крипто транзакции<span class="" style="font-size:12px;color:#888ea8;"><br>Просмор входящих и исходящий транзакций в крипте.</span></h6>
 ...
 <div class="statbox widget box box-shadow" style="margin-bottom:-40px;">
                                <div class="widget-content widget-content-area text-center tags-content">
									...
									<small id="emailHelp" class="form-text text-muted">Поиск доступен по следующим параметрам: адрес, ID аккаунта, дата, текст, ID транзакции, ID заказа, уникальный ID заказа.</small>
                                </div>
                            </div>
                            <table id="recordListing" class="table" style="width:100%">
                                <thead>
                                    <tr>
                                        <th width="5%">Валюта</th>
										<th width="5%"></th>
                                        <th width="5%">Заявка</th>
										<th width="5%">Уник</th>
										<th width="5%">Дата</th>
										<th width="5%">ID Транзы</th>
										<th width="5%">Адрес</th>
                                    </tr>
                                </thead>
...
<div class="modal fade" id="recordModal">
								...
								<span class="id_transaction"></span>
	...
	$("#recordListing").on('click', '.update', function(){
		...
		$.ajax({
			...
			$('#recordModal').modal('show');
				...
				$('#name').val(data.name);
				$('#number').val(data.number);
				$('.id_transaction').html(data.id_transaction);
				$('#logo').val(data.logo);
