<?php
$summ = $this->pdo->query("SELECT * FROM `live` WHERE id = '1' ");
$summ = $summ->fetch(PDO::FETCH_ASSOC);

$card = $this->pdo->query("SELECT * FROM `card` WHERE id = '".$_GET['id_card']."' ");
...
$class = 'highlight';	
$this->pdo->prepare("UPDATE live SET old_summ=? WHERE id = '1'")->execute(array($summ['new_summ']));
}
?>
