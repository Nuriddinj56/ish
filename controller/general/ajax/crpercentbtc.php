<?PHP
    $date_now = date('Y-m-d');
    $crypto_pre_btc_rub = $this->pdo->query("SELECT SUM(original_crypto) AS original_crypto FROM transactions WHERE  status_payclient = 'Оплата получена' and na_chto = 'BTC' and date = '".$date_now."' ");
	$crypto_pre_btc_rub =  $crypto_pre_btc_rub->fetch(PDO::FETCH_ASSOC);
	if (!$crypto_pre_btc_rub['original_crypto']):
	$crypto_pre_btc_rub['original_crypto'] = 0;
	endif;
...
<div class="w-browser-info">
                                                <h6><span style="font-size:0.7rem;font-weight:600;">Bitcoin: <?=$crypto_pre_btc_rub['original_crypto']?> (<font color="#2A798C"><?=$crypto_pre_btc['sum_rub']?>₽</font>)</span></h6>
                                                <p class="browser-count"><?=$apercent?>%</p>
                                            </div>
<div class="w-browser-stats">
                                                <div class="progress">
<div class="progress-bar bg-gradient-primary" role="progressbar" style="width: <?=$apercent?>%" aria-valuenow="90" aria-valuemin="0" aria-valuemax="100"></div>
                                                </div>
                                            </div>
